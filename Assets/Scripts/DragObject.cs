...
public class DragObject : MonoBehaviour
{
    ...
    private void OnTriggerStay2D(Collider2D collision)
    {
        ...
        if (mouseReleased && thisGameobjName == "Dragon1" && thisGameobjName == collisionGameobjName)
        {
            Instantiate(Resources.Load("Dragon2_Object"), transform.position, Quaternion.identity);
            mouseReleased = false;
            Destroy(collision.gameObject);
            Destroy(gameObject);
        }

        else if (mouseReleased && thisGameobjName == "Dragon2" && thisGameobjName == collisionGameobjName)
        {
            Instantiate(Resources.Load("Dragon3_Object"), transform.position, Quaternion.identity);
            mouseReleased = false;
            Destroy(collision.gameObject);
            Destroy(gameObject);
        }
